...
一个开箱即用的 **视频理解 + 多分支上下文对话** 前端工具，基于阿里云百炼（DashScope）大模型。目前**首个**基于百炼平台的可用前端。
...
## 二、支持模型与官方单价（不一定准确，仅供参考）（华北2·北京，每百万 Token）
...
- 实测参考：7.4 秒 720p 短视频 ≈ 8,800 Token ≈ **¥0.03**。10分钟左右纪录片，共1800余帧，实际单次对话花费在3-5人民币左右。
- 未收录的模型按通用规则估算，界面会提示；实际消耗以每次 API 返回的 `usage` 为准
- 理解后的对话成本本程序无法估算，程序提供的价格仅供参考。
...
仓库源码不含内置 Python。为方便普通用户，在 **Releases** 附上自包含构建包：在上面源码基础上加入 `python/`（Windows 官方便携版 Python 3.14.7）并清空 `config.json`，`启动.cmd` 会自动优先使用内置 Python，实现目标电脑**免安装双击即用**。
